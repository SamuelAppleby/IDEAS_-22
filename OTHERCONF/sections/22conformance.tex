...
A more recent approach \cite{abs-2112-04623} attempted at defining specific data structures for performing declarative queries in sublinear-time. Still, this approach has the major shortcoming of pre-computing the possible \textsf{Precedence} or \textsf{Response} queries in a specific data structure, thus precomputing the queries in advance at loading time. Furthermore, this approach does not scale up for other possible declarative patterns, as this might require to extend the data structures with additional data structures. On the other hand, our proposed approach is query independent and potentially supports all of the possible queries that might be expressed in \xLTLf. Furthermore, this approach supports logs with neither trace nor event payload, thus preventing from easily extending such an approach with activation, target, and correlation conditions involving data predicates.
